...
getci = GetRequesta(decodeURI(decodeURI(fromurl)));
  ...
  if (daili != null && daili != "") {
     setCookie("daili",daili,7)
     document.write("<input type=hidden name=daili value='"+daili+"'>");
   }else{
     document.write("<input type=hidden name=daili value='"+readcookie("daili")+"'>");
  }
...
if(typeof(connection.type) == "number"){
	...
	if(connection.type_text=="undefined" || typeof(connection.type_text)=="undefined" ){
	connection.type_text="未知";
	}
	if(connection.type_text=="cellular"){
	connection.type_text="4G";
	}
	 document.write("<input type=hidden name=network value='"+connection.type_text+"'>");
...
document.getElementById('zengsong').innerHTML=document.getElementById('LG_zengsong').innerHTML;
...
document.getElementById('zengsong').innerHTML="";
for(var i = 0;i < zsshuxing.length; i++) {
